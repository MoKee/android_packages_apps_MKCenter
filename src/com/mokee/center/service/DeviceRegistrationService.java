        ...
        for (int slotId = 0; slotId < telephonyManager.getSimCount(); slotId ++) {
            ...
            if (phone != null) {
                if (phone.getPhoneType() == TelephonyManager.PHONE_TYPE_CDMA) {
                    if (CommonUtils.isValid(phone.getMeid())) stringBuffer.append(phone.getMeid()).append(",");
                    if (phone.getLteOnCdmaMode() == PhoneConstants.LTE_ON_CDMA_TRUE) {
                        if (CommonUtils.isValid(phone.getImei())) stringBuffer.append(phone.getImei()).append(",");
                    }
                } else {
                    if (CommonUtils.isValid(phone.getImei())) stringBuffer.append(phone.getImei()).append(",");
                }
